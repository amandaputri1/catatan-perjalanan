{{-- css --}}
<link rel="shortcut icon" href="{{ asset('img/inventaris.png') }}" type="image/png">


{{-- datatables --}}
<link rel="stylesheet" href="{{ asset('template/dist/assets/extensions/simple-datatables/style.css') }}">
<link rel="stylesheet" href="{{ asset('template/dist/assets/compiled/css/simple-datatable.css') }}">
<link rel="stylesheet" href="{{ asset('template/dist/assets/compiled/css/table-datatable.css') }}">

{{-- css --}}
<link rel="stylesheet" href="{{ asset('template/dist/assets/compiled/css/app.css') }}">
<link rel="stylesheet" href="{{ asset('template/dist/assets/compiled/css/app-dark.css') }}">
<link rel="stylesheet" href="{{ asset('template/dist/assets/compiled/css/iconly.css') }}">


{{-- dripicons --}}
<link rel="stylesheet" href="{{ asset('template/dist/assets/extensions/@icon/dripicons/dripicons.css') }}">
<link rel="stylesheet" href="{{ asset('template/dist/assets/compiled/css/ui-icons-dripicons.css') }}">

{{-- fontawesome --}}
<link rel="stylesheet" href="{{ asset('template/dist/assets/extensions/@fortawesome/fontawesome-free/css/all.min.css') }}">

<link rel="stylesheet" href="{{ asset('views/style/css.blade.php') }}">
<style>
    .background-class {
    background-image: url('template/dist/assets/static/images/bg.jpg');
    }

    .fontawesome-icons {
        text-align: center;
    }

    article dl {
        background-color: rgba(0, 0, 0, .02);
        padding: 20px;
    }

    .fontawesome-icons .the-icon {
        font-size: 24px;
        line-height: 1.2;
    }

    .card .card-text {
        height: 15%
    }
</style>